...
EFI_STATUS SaveMemoryMap(struct MemoryMap* map, EFI_FILE_PROTOCOL* file) {
  ...
  for (iter = (EFI_PHYSICAL_ADDRESS)map->buffer,
      i = 0; iter < (EFI_PHYSICAL_ADDRESS)map->buffer + map->map_size; iter += map->descriptor_size,
      i++) {
...
EFI_STATUS EFIAPI UefiMain(
  ...
  Print(L"Resolution: %ux%u, Pixel Format: %s, %u pixel/line\n",
...
*/
  ...
  UINT64 kernel_first_add, kernel_last_addr;
  CalcLoadAddressRange(kernel_ehdr, &kernel_first_add, &kernel_last_addr);

  UINTN num_pages = (kernel_last_addr - kernel_first_add + 0xfff) / 0x1000;
  status = gBS->AllocatePages(AllocateAddress, EfiLoaderData, num_pages, &kernel_first_add);
  if (EFI_ERROR(status)) {
  ...
  Print(L"kernel: 0x%0lx - 0x%0lx\n", kernel_first_add, kernel_last_addr);
  ...
  // Read Volume image end


  // Exit Boot Service begin
  ...
  // Exit Boot Service end

  // Call Kernel begin
  UINT64 entry_addr = *(UINT64*)(kernel_first_add + 24);
