...
void MouseObserver(uint8_t buttons, int8_t displacement_x, int8_t displacement_y) {
  ...
  } else if (previous_left_pressed && ! left_pressed) { // ボタン離し瞬間
...
void SwitchEhci2Xhci(const pci::Device& xhc_dev) {
  ...
  for (int i = 0; i < pci::num_device; ++i) {
    if (pci::devices[i].class_code.Match(0xcu, 0x03u, 0x20u) && 0x8086 == pci::ReadVendorId(pci::devices[i])) {
...
extern "C" void KernelMainNewStack(const FrameBufferConfig& frame_buffer_config_ref, const MemoryMap& memory_map_ref) {
  ...
  for (int i = 0; i < pci::num_device; ++i) {
    if (pci::devices[i].class_code.Match(0x0cu, 0x03u, 0x30u)) {
      ...
      if (0x8086 == pci::ReadVendorId(*xhc_dev)) { // Intel PCI devices have common vendor id:0x8086
        break;
      }
    }
  }
  ...
  const uint8_t bsp_local_apic_id = *reinterpret_cast<const uint32_t*>(0xffe0020) >> 24;
  ...
  DrawWindow(*main_window->Writer(), "HelloWindow!");
