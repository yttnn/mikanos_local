...
WithError<size_t> SetupPageMap(PageMapEntry* page_map, int page_map_level, LinerAddress4Level addr, size_t num_4kpages) {
  while (num_4kpages > 0) {
...
Terminal::Terminal() {
  window_ = std::make_shared<ToplevelWindow>(kColumns*8 + 8 + ToplevelWindow::kMarginX, kRows*16 + 8 + ToplevelWindow::kMarginY, screen_config.pixel_format, "MikanTerm");
...
Rectangle<int> Terminal::HistroyUpDown(int direction) {
